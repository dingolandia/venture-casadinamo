...
2. Run `npm start` to build in development mode.
...
1. Open the [environment.prod.ts file](./src/environments/environment.prod.ts) and change the values to match your deployed Vendure server.
2. Open the [angular.json file](./angular.json) and set `baseHref` to match the public path where the storefront will be mounted. For example, while the storefront is temporarily published under `/store`, use:
    ```diff
      "production": {
    -    "baseHref": "/",
    +    "baseHref": "/store/",
    ```
   When the storefront moves to the site root, switch `baseHref` back to `'/'`.
